...
function phanTich_URL_chiTietSanPham() {
   ...
   // document.getElementById('detailPromo').innerHTML = getDetailPromo(sanPham);
    ...
    var s = addThongSo('Giáo viên', sanPham.detail.gv);
    s += addThongSo('Trình độ giảng viên', sanPham.detail.trinhdo);
    s += addThongSo('Kinh nhiệm giảng dậy', sanPham.detail.kinhnghiem);
    s += addThongSo('Thời gian một tiết', sanPham.detail.time);
    s += addThongSo('Số buổi trong tuần', sanPham.detail.tuan);
    s += addThongSo('Tổng số tiết', sanPham.detail.thoigian);
    s += addThongSo('Địa điểm học', sanPham.detail.diadiem);
    s += addThongSo('Chú thích', sanPham.detail.chuthich);
    s += addThongSo('Giá khóa luyện thi', price +' VND');
    info.innerHTML = s;
